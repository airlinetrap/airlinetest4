...
EntityManager em;
	...
	public List<Passenger> viewPassengerById(int userId) {
		TypedQuery<Passenger> passenger=em.createQuery("select p from Passenger p where p.user.userId = :uId  ",Passenger.class); 
		passenger.setParameter("uId", userId);
		return passenger.getResultList();
	}
	@Override
	public int noOfSeatsBooked(int flightId, LocalDate dateOfTravel) {
		...
		Query query = em.createQuery("select count(p) from Passenger p where p.flight.flightNo=:fId and p.dateOfTravel=:dOFT");
		query.setParameter("fId", flightId);
		query.setParameter("dOFT", dateOfTravel);
	...
	public int findFlightInPassenger(int flightId) {
		// TODO Auto-generated method stub
		Query query = em.createQuery("select count(p) from Passenger p where p.flight.flightNo=:fId");
		query.setParameter("fId", flightId);
		long count = (long) query.getSingleResult();
		return (int)count;
