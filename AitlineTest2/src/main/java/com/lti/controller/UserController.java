...
public class UserController {
	...
	public Status deleteFlight(@RequestBody DeleteFlightDto deleteFlight)
	{
		try {
			flightService.deleteAFlight(deleteFlight.getFlightId());
			Status status = new Status();
			status.setStatus(StatusType.SUCCESS);
			status.setMessage("Flight Deleted Successfully");
			return status;
		} catch (FlightServiceException e) {
			// TODO: handle exception
			Status status = new Status();
			status.setStatus(StatusType.FAILURE);
			status.setMessage("Flight cannot be deleted");
			return status;
		}
	}
	...
	@PostMapping(path = "/noofseatsbooked")
	public int noOfSeatsBooked(@RequestBody NoOfSeats noOfSeats) {
		return passengerService.noOfSeatsBooked(noOfSeats.getFlightNo(),noOfSeats.getDateOfTravel());
	}
